...
document.addEventListener("DOMContentLoaded", function () {
    ...
    calculateBtn.addEventListener("click", function () {
        ...
        //********************************* Input validation ****************************************
        if(!companyName || isNaN(teamSize) || isNaN(revenue) || isNaN(profitMargin) || isNaN(growthRate)) {
            result.innerHTML = "Please enter valid data for all fields.";
            return;
        }
        //********************************** company valuation **************************************
        const valuation = (revenue * (1 + growthRate)) / (1 - profitMargin);
        result.innerHTML = `Company Name: ${companyName}<br>Valuation: $${valuation.toFixed(2)}`;
    });
});
